...
import MapPinInput from "@/app/components/pages/MapPinInput";
...
export default function AdminProperties() {
  ...
  const handleAddPropertyImage = async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `properties/${Date.now()}_${propertyImageFile.name}`
      );
  ...
  const handleAddRoomImage = async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `rooms/${Date.now()}_${roomImageFile.name}`
      );
  ...
  const handleEdit = (property: IProperty) => {
    setForm({
      ...
      location: Array.isArray(property.location)
        ? property.location
        : [
            {
              lat: 0,
              lng: 0,
              address: property.location as string,
              shareUrl: "",
            },
          ],
            ...
            /> */}
                <MapPinInput
                  onLocationSelect={(location) => {
                    setForm((prev) => ({
                      ...prev,
                      location: [location], // Replace existing location with new one
                    }));
                  }}
                  initialLocation={
                    form.location.length > 0 ? form.location[0] : undefined
                  }
                />
              </div>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start gap-2">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 flex items-center gap-1 mt-1">
                          ...
                          {Array.isArray(property.location) && property.location.length > 0
